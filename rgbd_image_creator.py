...
def extract_edof(data, idx, fname):
    ...
    outfname = ('tmp/' + ''.join(fname.split('.')[:-1])) + '-1_depth.png'
...
def scan_segment(data, idx, fname, segment_index):
    ...
    if data.find(bytes([0xff, 0xd8]), idx) > 0:
        ...
        if error < 15000:
            ...
            print("\t* Error larger than 15000, skipping")
    ...
    while i < len(data):
        if data[i] == 0xff:
            ...
            if data[i + 1] == 0xda:
                ...
                if segment_index == 1:
                    outfname = ('tmp/' + ''.join(fname.split('.')[:-1])) + ('-%d.JPG' % segment_index)
...
def createRGDBImage(fname):
    print("Creating RGBD file")
    outfname_depthmap = ('tmp/' + ''.join(fname.split('.')[:-1])) + '-1_depth.png'
    outfname_color = ('tmp/' + ''.join(fname.split('.')[:-1])) + '-1.JPG'
    dmapImage = Image.open(outfname_depthmap)
    colorImage = Image.open(outfname_color)

    # we resize to color image scale
    width, height = colorImage.size
    dmapImage = dmapImage.resize((width, height), Image.ANTIALIAS)

    # concat images
    dst = Image.new('RGB', (colorImage.width + dmapImage.width, colorImage.height))
    dst.paste(colorImage, (0, 0))
    dst.paste(dmapImage, (colorImage.width, 0))
    dst.save(('out/' + ''.join(fname.split('.')[:-1])) + '-RGBD.JPG')
...
def main(fname):
    ...
    while True:
        ...
        if r == -1:
            if segment_index > 1:
                if extract_edof(data, idx, fname):
                    # segment and depth map extracted, now merge both
                    return createRGDBImage(fname)
            else:
                return False
...
if __name__ == "__main__":
    print("Huawei Camera EDOF Extractor - Looking Glass Portrait RGDB Photo Extension \n")
